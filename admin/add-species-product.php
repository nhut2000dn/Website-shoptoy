<?php
  ...
  if (isset($_POST["submit-btn"])) {
    ...
    if (strlen($_POST["nameSpecies"]) == "0") {
        $error.= "<li>Tên Loại không được để trống !</li>";
        $check = false;
    }
    if (strlen($_POST["describeSpecies"]) == 0) {
        $error.= "<li>Mô tả không được để trống !</li>";
        $check = false;
    }
    if ($check == true) {
        $nameSpecies = $_POST["nameSpecies"];
        $describeSpecies = $_POST["describeSpecies"];
        $queryInsert  = "INSERT INTO `pd_species`(`name_species`, `url_name_species`, `describe_species`) VALUES ('$nameSpecies', '$urlNameSpecies', '$describeSpecies');";
        // $queryInsert  = "UPDATE account SET first_name=N'$ho', last_name=N'$ten', GT='$gt', date_of_birth='$dateOfBirth', status='$status', classify='$classify' WHERE id='$idSpecies'";
        $result   = mysqli_query($connection,$queryInsert)or die("loi cap nhat".mysqli_error($con));
        echo $queryInsert;
        if($result) {
            echo "<script type='text/javascript'>alert('Thêm Loại thanh cong');</script>";
  ...
  <div class="main-left">
    ...
    <div class="main-right">
      <section class="name-main-right">
        <p class="edit">Thêm Loại</p>
        <p class="border-bottom"></p>
      </section>
      <form action="add-species-product.php" method="post">
        <table>
            <tbody>
                <tr><td><span>Tên Loại</span></td></tr>
                <tr><td><input name="nameSpecies" type="text" placeholder="Nhập Tên Loại"></td></tr>
                ...
                <tr>
                    <td>
                        ...
                        <textarea name="describeSpecies" rows="7" cols="70"></textarea>
                    </td>
                </tr>
                <tr>
                    <td>
