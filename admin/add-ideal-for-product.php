<?php
  ...
  if (isset($_POST["submit-btn"])) {
    ...
    if (strlen($_POST["nameIdealFor"]) == "0") {
        $error.= "<li>Tên Giới Tính Thích Hợp không được để trống !</li>";
        $check = false;
    }
    if ($check == true) {
        $nameIdealFor = $_POST["nameIdealFor"];
        $queryInsert  = "INSERT INTO `pd_ideal_for`(`name_ideal_for`, `url_name_ideal`) VALUES ('$nameIdealFor', '$urlNameIdealFor');";
        // $queryInsert  = "UPDATE account SET first_name=N'$ho', last_name=N'$ten', GT='$gt', date_of_birth='$dateOfBirth', status='$status', classify='$classify' WHERE id='$idIdealFor'";
        $result   = mysqli_query($connection,$queryInsert)or die("loi cap nhat".mysqli_error($con));
        echo $queryInsert;
        if($result) {
            echo "<script type='text/javascript'>alert('Thêm Giới Tính Thích Hợp thành công');</script>";
  ...
  <div class="main-left">
    ...
    <div class="main-right">
      <section class="name-main-right">
        <p class="edit">Thêm Loại Giới Tính Thích Hợp</p>
        <p class="border-bottom"></p>
      </section>
      <form action="add-ideal-for-product.php" method="post">
        <table>
            <tbody>
                <tr><td><span>Tên Giới Tính Thích Hợp</span></td></tr>
                <tr><td><input name="nameIdealFor" type="text" placeholder="Nhập Tên Giới Tính Thích Hợp"></td></tr>
                <tr>
                    <td>
                        <button type="submit" name="submit-btn">Add IdealFor</button>
                    </td>
                </tr>
            </tbody>
            <td>
