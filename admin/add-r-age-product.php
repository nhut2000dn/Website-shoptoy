<?php
  ...
  if (isset($_POST["submit-btn"])) {
    $urlNameRAge = strtolower(vn_to_str($_POST["nameRAge"]));
    if (strlen($_POST["nameRAge"]) == "0") {
        $error.= "<li>Tên Loại Tuổi Thích Hợp không được để trống !</li>";
        $check = false;
    }
    if ($check == true) {
        $nameRAge = $_POST["nameRAge"];
        $queryInsert  = "INSERT INTO `pd_recommended_age`(`name_r_age`, `url_name_r_age`) VALUES ('$nameRAge', '$urlNameRAge');";
        // $queryInsert  = "UPDATE account SET first_name=N'$ho', last_name=N'$ten', GT='$gt', date_of_birth='$dateOfBirth', status='$status', classify='$classify' WHERE id='$idRAge'";
        $result   = mysqli_query($connection,$queryInsert)or die("loi cap nhat".mysqli_error($con));
        echo $queryInsert;
        if($result) {
            echo "<script type='text/javascript'>alert('Thêm Loại Tuổi Thích Hợp thành công');</script>";
            echo'<meta http-equiv="refresh" content="0; url=recommended-age-product.php" />';
        }
    }
  }
?>
  ...
  <div class="main-left">
    ...
    <div class="main-right">
      <section class="name-main-right">
        <p class="edit">Thêm Loại Loại Tuổi Thích Hợp</p>
        <p class="border-bottom"></p>
      </section>
      <form action="add-r-age-product.php" method="post">
        <table>
            <tbody>
                <tr><td><span>Tên Loại Tuổi Thích Hợp</span></td></tr>
                <tr><td><input name="nameRAge" type="text" placeholder="Nhập Tên Loại Tuổi Thích Hợp"></td></tr>
                <tr>
                    <td>
                        <button type="submit" name="submit-btn">Add RAge</button>
                    </td>
                </tr>
            </tbody>
            <td>
