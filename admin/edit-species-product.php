<?php
    ...
    $idSpecies = $_GET["id"];
    $query_check  = 'SELECT * FROM pd_species WHERE id="'.$idSpecies.'"';
    ...
    while ($row=mysqli_fetch_array($result_check, MYSQLI_ASSOC)) {
        ...
        $describeSpecies = $row["describe_species"];
    }
    $error;
    $check = true;
    if (isset($_POST["submit-btn"])) {
        $urlNameSpecies = strtolower(vn_to_str($_POST["nameSpecies"]));
        if (strlen($_POST["nameSpecies"]) == "0") {
            $error.= "<li>Họ không được để trống !</li>";
            $check = false;
        }
        if (strlen($_POST["describeSpecies"]) == 0) {
            $error.= "<li>Tên không được để trống !</li>";
            $check = false;
        }
        if ($check == true) {
            $nameSpecies = $_POST["nameSpecies"];
            $describeSpecies = $_POST["describeSpecies"];
            $queryUpdate  = "UPDATE pd_species SET name_species=N'$nameSpecies', url_name_species=N'$urlNameSpecies', describe_species=N'$describeSpecies' WHERE id='$idSpecies'";
            // $queryUpdate  = "UPDATE account SET first_name=N'$ho', last_name=N'$ten', GT='$gt', date_of_birth='$dateOfBirth', status='$status', classify='$classify' WHERE id='$idSpecies'";
            ...
            if($result) {
                ...
                echo'<meta http-equiv="refresh" content="0; url=species-product.php" />';
            }
        }
    }
?>
...
<body>
  ...
  <div class="main-left">
    ...
    <div class="main-right">
      ...
      <form action="<?php echo "edit-species-product.php?id=$idSpecies" ?>" method="post">
        <table>
            <tbody>
                <tr><td><span>Tên Loại</span></td></tr>
                <tr><td><input name="nameSpecies" type="text" placeholder="Nhập Tên Loại" value="<?php if(isset($nameSpecies)) echo $nameSpecies;?>"></td></tr>
                <tr><td><span>Mô Tả</span></td></tr>
                <tr>
                    <td>
                        <!-- <input name="ten" type="text" placeholder="Nhập họ của bạn" value="<?php if(isset($describeSpecies)) echo $describeSpecies;?>"> -->
                        <textarea name="describeSpecies" rows="7" cols="70"><?php if(isset($describeSpecies)) echo $describeSpecies;?></textarea>
                    </td>
                </tr>
                <tr>
                    <td>
