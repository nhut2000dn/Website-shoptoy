<?php
  ...
  $query_selectStatus0  = 'SELECT * FROM shop_toy.contact WHERE contact.status = 0 ORDER BY id DESC';
  ...
  while($row=mysqli_fetch_array($result_check,MYSQLI_ASSOC)) {
    ...
    $html.='<td>'.$i.'</td>';
    $html.='<td>'.$row["id"].'</td>';
    $html.='<td>'.$row["email_contact"].'</td>';
    $html.='<td class="button"><a href="contact.php?id='.$row["id"].'" class="btn-edit">View</a></td>';
    $html.='<td class="button"><a onclick="return confirmdelete()" href="c-delete-contact.php?id='.$row["id"].'" class="btn-delete">Delete</a></td>';
    $html.='</tr>'; 
  }

  $query_selectStatus1  = 'SELECT * FROM shop_toy.contact WHERE contact.status = 1 ORDER BY id DESC';
  ...
  while($row=mysqli_fetch_array($result_check,MYSQLI_ASSOC)) {
    ...
    $html2.='<td>'.$i.'</td>';
    $html2.='<td>'.$row["id"].'</td>';
    $html2.='<td>'.$row["email_contact"].'</td>';
    $html2.='<td class="button"><a href="contact.php?id='.$row["id"].'" class="btn-edit">View</a></td>';
    $html2.='<td class="button"><a onclick="return confirmdelete()" href="c-delete-contact.php?id='.$row["id"].'" class="btn-delete">Delete</a></td>';
    $html2.='</tr>'; 
  }
  ...
  if(isset($_GET["id"])) {
    $id = $_GET["id"];
    $htmlContentComment1;
    $queryUpdate = "UPDATE contact SET status='1' WHERE contact.id='$id'";
    $result   = mysqli_query($connection,$queryUpdate)or die("loi cap nhat".mysqli_error($con));
    $query_selectContent  = "SELECT * FROM shop_toy.contact WHERE contact.id='$id';";
    $result_check = mysqli_query($connection,$query_selectContent) or die ("loi".mysqli_error($connection));
    while($row=mysqli_fetch_array($result_check,MYSQLI_ASSOC)) {

     $htmlContentComment1.='<p>' . $row["content_message"] .'</p>';
    }
    $htmlContentComment2 = '
      <div class="view" id="view">
        <div class="modal__content">
          <a href="contact.php" class="modal__close">&times;</a>
        ...
        <section class="name-main-right">
          <div class="title-bar">
            <div class="title-left">
              <p class="dashboard">Control Table - contact</p>
            </div>
